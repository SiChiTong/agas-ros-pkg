...
THIS::THIS ( std::vector< KeyPoint >* keyPoints1, std::vector< KeyPoint >* keyPoints2, std::list< KeyPointMatch >& matches )
{
  ...
  m_Matches = matches;
  m_Success = false;
  m_MaxReprojectionError = 1; // TODO Config::getFloat( "ObjectRecognition.Ransac.fMaxReprojectionError" );
}
...
bool THIS::computeFundamentalMat()
{
  ...
  int method = 0;
  switch (CV_FM_7POINT) // TODO Config::getInstance()->getInt( "ObjectRecognition.FundamentalMat.iMethod" ))
  {
    case 0 :
      method = CV_FM_7POINT;
      break;
    ...
    case 3:
      ...
      break;
    default:
      // TRACE_ERROR("Undefined methode to find fundamental matrix"); // TODO use ros
      break;
  }
  CvMat* status;
...
//int cvFindFundamentalMat(const CvMat* points1, const CvMat* points2, CvMat* fundamental_matrix, int method=CV_FM_RANSAC, double param1=1., double param2=0.99, CvMat* status=NULL)
  ...
  // TRACE_INFO("Status = " << m_Success );  // TODO use ros
